...
#define ID_NAVDOP   0x0104      /* ubx message id: nav dop info */
...
#define ID_SECUNIQ  0x2703      /* ubx message id: Unique Chip ID */
...
static int decode_rxmraw(raw_t *raw)
{
	...
	for (i = 0, p += 8; i<nsat&&i<MAXOBS; i++, p += 24) {
		...
		if (!(sat = satno2satIndex(MINPRNSBS <= prn ? SYS_SBS : SYS_GPS, prn))) {
...
static int decode_rxmrawx_old(raw_t *raw)
{
	gtime_t time;
...
static int decode_rxmrawx(raw_t *raw)
{
	...
	if (raw->outtype) {
		sprintf(raw->msgtype, "UBX RXM-RAWX  (%4d): nsat=%d", raw->len,
			U1(p + 11));
	}
	/* time tag adjustment option (-TADJ) */
	if ((q = strstr(raw->opt, "-TADJ="))) {
...
static int decode_navtime(raw_t *raw)
{
	...
	if ((U1(p + 11) & 0x03) == 0x03) {
		raw->time = gpst2time(week, itow*1E-3 + ftow*1E-9);
		timeset(gpst2utc(raw->time));
		raw->leaps = leaps;
		raw->week = week;
		raw->sow = itow*1E-3 + ftow*1E-9;
		raw->ubxpvt |= 4;
	}
	return 0;
}
/* decode ubx-nav-dop: vdop pdop hdop vdop ---------------------------------*/
static int decode_navdop(raw_t *raw)
{
	unsigned char *p = raw->buff + 6;
	raw->dop[0] = U2(p + 8)*0.01;
	raw->dop[1] = U2(p + 6)*0.01;
	raw->dop[2] = U2(p + 12)*0.01;
	raw->dop[3] = U2(p + 10)*0.01;
	raw->ubxpvt |= 1;
	return 0;
}
...
{
	unsigned char *p = raw->buff + 6;
	raw->blh[1] = I4(p + 24)*1e-7;
	raw->blh[0] = I4(p + 28)*1e-7;
	raw->blh[2] = I4(p + 32)*0.001;
	raw->undulation = raw->blh[2] - I4(p + 36)*0.001;
	raw->blh[3] = raw->blh[4] = U4(p + 40)*0.000707107;
	raw->blh[5] = U4(p + 44)*0.001;
	raw->venu[0] = I4(p + 52)*0.001;
	raw->venu[1] = I4(p + 48)*0.001;
	raw->venu[2] = -I4(p + 56)*0.001;
	raw->venu[3] = raw->venu[4] = raw->venu[5] = U4(p + 68)*0.000577350;
	raw->numSV = U1(p + 23);
	raw->fixstatus = U1(p + 20) == 0 ? 1 : U1(p + 20);
	raw->ep[0] = U2(p + 4);
	raw->ep[1] = U1(p + 6);
	raw->ep[2] = U1(p + 7);
	raw->ep[3] = U1(p + 8);
...
static void cmpuniqid(short int *q1, short int *q2)
{
	int i;
	for(i=0;i<5;i++)
	{
...
/* decode ubx-sec-unique ---------------------------------*/
static int decode_secuniq(raw_t *raw)
{
	double ver;
	int i;
	short int uniqueid[5];
	unsigned char *p = raw->buff + 6;
	ver = U1(p);
	for (i = 0; i<5; i++)
	{
	...
	FILE *fp = fopen("E:\\uniqid.txt", "r");
	char *q = NULL;
	char buff[20];
	short int uniqid0[5];
	fgets(buff, 20, fp);
	for (i=0,q = strtok(buff, " "); q; q = strtok(NULL, " "),i++)
	{
		uniqid0[i] = (short int) strtol(q, NULL, 16);
	}
	fclose(fp);
	cmpuniqid(uniqid0, uniqueid);
	return 0;
}
...
static int decode_ubx(raw_t *raw)
{
	...
	case ID_NAVDOP: return decode_navdop(raw);
	case ID_NAVPVT: return decode_navpvt(raw);
	case ID_SECUNIQ: return decode_secuniq(raw);
...
extern void gen_uniqidbuff(short int *uniqid, unsigned char *uniqidbuff)
{
	...
	for (i = 0; i<8; i++)
	{
		setU1(uniqidbuff + i, (unsigned char)(uniqid[i]));
	}
}
