...
static int openfile_(file_t *file, gtime_t time, char *msg)
{
	...
	if (file->timetag) { /* output/sync time-tag */
		...
		if (file->mode&STR_MODE_R) {
			...
			sprintf(tagh, "TIMETAG RTKLIB %s", versionNum);
...
* stream path ([] options):
*
*   STR_SERIAL   port[:brate[:bsize[:parity[:stopb[:fctr]]]]]跟着更多未定义的参考到
...
extern int strread(stream_t *stream, unsigned char *buff, int n)
{
	...
	case STR_FILE: {
		if (feof(((file_t*)stream->port)->fp))
		{
			
			return -1;
		}
		nr = readfile((file_t*)stream->port, buff, n, msg); break;
	}
...
extern void strsenduniqidcmd(stream_t *str, const char *cmd)
{
	short int uniqid[8] = { 181,98,39,3,0,0,42,165 };
	unsigned char uniqidbuff[8];
	gen_uniqidbuff(uniqid, uniqidbuff);
	strwrite(str, uniqidbuff, 8);
}
