...
//obs stringline
#define stringVersion            "RINEX VERSION / TYPE"
#define stringRunBy              "PGM / RUN BY / DATE"
#define stringComment            "COMMENT"
#define stringMarkerName         "MARKER NAME"
#define stringMarkerNumber       "MARKER NUMBER"
#define stringMarkerType         "MARKER TYPE"
#define tringObserver            "OBSERVER / AGENCY"
#define stringReceiver           "REC # / TYPE / VERS"
#define stringAntennaType        "ANT # / TYPE"
#define stringAntennaPosition    "APPROX POSITION XYZ"
#define stringAntennaDeltaHEN    "ANTENNA: DELTA H/E/N"
#define stringAntennaDeltaXYZ    "ANTENNA: DELTA X/Y/Z"
#define stringAntennaPhaseCtr    "ANTENNA: PHASECENTER"
#define stringAntennaBsightXYZ   "ANTENNA: B.SIGHT XYZ"
#define stringAntennaZeroDirAzi  "ANTENNA: ZERODIR AZI"
#define stringAntennaZeroDirXYZ  "ANTENNA: ZERODIR XYZ"
#define stringCenterOfMass       "CENTER OF MASS: XYZ"
#define stringNumObs             "# / TYPES OF OBSERV"      // R2
#define stringSystemNumObs       "SYS / # / OBS TYPES"
#define stringWaveFact           "WAVELENGTH FACT L1/2"     // R2
#define stringSigStrengthUnit    "SIGNAL STRENGTH UNIT"
#define stringInterval           "INTERVAL"
#define stringFirstTime          "TIME OF FIRST OBS"
#define stringLastTime           "TIME OF LAST OBS"
#define stringReceiverOffset     "RCV CLOCK OFFS APPL"
#define stringSystemDCBSapplied  "SYS / DCBS APPLIED"
#define stringSystemPCVSapplied  "SYS / PCVS APPLIED"
#define stringSystemScaleFac     "SYS / SCALE FACTOR"
#define stringSystemPhaseShift   "SYS / PHASE SHIFTS"
#define stringGlonassSlotFreqNo  "GLONASS SLOT / FRQ #"
#define stringLeapSeconds        "LEAP SECONDS"
#define stringNumSats            "# OF SATELLITES"
#define stringPrnObs             "PRN / # OF OBS"
#define stringEoH                "END OF HEADER"

//nav stringline
#define stringIonoCorr           "IONOSPHERIC CORR"
#define stringTimeSysCorr        "TIME SYSTEM CORR"
#define stringDeltaUTC           "DELTA-UTC: A0,A1,T,W"
#define stringCorrSysTime        "CORR TO SYSTEM TIME"
#define stringDUTC               "D-UTC A0,A1,T,W,S,U"
#define stringIonAlpha           "ION ALPHA"
#define stringIonBeta            "ION BETA"
...
extern void init_rtcm(rtcm_t *rtcm)
{
	...
	for (i = 0; i < MAXTECLAYER; i++)
		for (j = 0; j < 3 + MAXTECCOEFF; j++)
			rtcm->ion.vtec[i][j] = 0.0;
	rtcm->ion.flag = 0;
	for (i = 0; i < 128; i++)
		rtcm->msg[i] = '\0';
	for (i = 0; i < 256; i++)
	...
	rtcm->obsflag = rtcm->ephsat = rtcm->nbyte = rtcm->nbit = rtcm->len = 0;
	for (i = 0; i < MAXSAT; i++) {
		for (j = 0; j < NFREQ; j++)
		{
			rtcm->cp[i][j] = 0.0;
			rtcm->lock[i][j] = rtcm->loss[i][j] = (unsigned char)0;
...
static int obsindex(obs_t *obs, gtime_t time, int sat)
{
	...
	for (j = 0; j<NFREQ; j++) {
		...
		obs->data[i].D[j] = 0.0;
		obs->data[i].SNR[j] = obs->data[i].LLI[j] = obs->data[i].code[j] = (unsigned char)0;
	}
	obs->n++;
	return i;
}
...
static int decode_head1001(rtcm_t *rtcm, int *sync)
{
	...
	int week;
	//	char *msg;
	int i = 24, stationID, nSat, type;
	type = getbitu(rtcm->buff, i, 12);
	i += 12;
	if (i + 52 < rtcm->len * 8)
	{
		stationID = getbitu(rtcm->buff, i, 12);
		i += 12;
	...
	if (!test_staid(rtcm, stationID)) return -1;


	if (rtcm->time.time == 0) rtcm->time = timeadd(timeget(), leapsecond(timeget()));//UTC?
	sec = time2gpst(rtcm->time, &week);
	if (tow<sec - 302400.0) tow += 604800.0;
	else if (tow>sec + 302400.0) tow -= 604800.0;
	rtcm->time = gpst2time(week, tow);

	return nSat;
}
static double adjcp(rtcm_t *rtcm, int sat, int nfreq, double cp)
{
	if (rtcm->cp[sat - 1][nfreq] == 0) {}
	else if (cp < rtcm->cp[sat - 1][nfreq] - 750) cp += 1500;
	else if (cp > rtcm->cp[sat - 1][nfreq] + 750) cp -= 1500;
	rtcm->cp[sat - 1][nfreq] = cp;
	return cp;
}
...
static void adjday_glot(rtcm_t *rtcm, double tod)
{
	...
	time = timeadd(timeadd(rtcm->time, -leapsecond(rtcm->time)), 10800.0); /* glonass time */
	sec = time2gpst(time, &week);
	tod_p = fmod(sec, 86400.0); sec -= tod_p;
	if (tod<tod_p - 43200.0) tod += 86400.0;
	else if (tod>tod_p + 43200.0) tod -= 86400.0;
	time = gpst2time(week, sec + tod);
	rtcm->time = timeadd(timeadd(time, -10800.0), leapsecond(timeadd(time, -10800.0)));
}

static void adjweek_gpst(rtcm_t *rtcm, double tow)
{
	int week;
	double sec;
	if (rtcm->time.time == 0) rtcm->time = timeadd(timeget(), leapsecond(timeget()));//UTC?
...
static int decode_type1004(rtcm_t *rtcm)
{
	...
	for (j = 0; j<nsat&&i + 125 <= rtcm->len * 8; j++) {
		...
		if (pr1 != (int)0xFFF80000)
		{
			pr1 = pr1*0.02 + amb*PRUNIT_GPS;
		}
		else
		{
			printf("P1 field error\n");
			return sync ? 0 : 1;
		}
...
//		printf("rtcm3 1005 length error: len=%d\n", rtcm->len);
		return -1;
	}
	/* test station id */
	if (!test_staid(rtcm, staid)) return -1;

	//	rtcm->sta.antennaDeltaType = 1; /* xyz */
	rtcm->sta.antennaDeltaType = 0; /* enu */
	for (j = 0; j<3; j++) {
		rtcm->sta.approxCoor[j] = rr[j] * 0.0001;
		rtcm->sta.antennaDelta[j] = 0.0;
	}
	rtcm->sta.hgt = 0.0;
	rtcm->sta.itrf = itrf;
	return 5;
...
//		printf("rtcm3 1006 length error: len=%d\n", rtcm->len);
		return -1;
	}
	/* test station id */
	if (!test_staid(rtcm, staid)) return -1;
	...
	rtcm->sta.antennaDelta[2] = rtcm->sta.hgt;
	rtcm->sta.itrf = itrf;
	return 5;
}
...
//		printf("rtcm3 1007 length error: len=%d\n", rtcm->len);
		return -1;
	}
	/* test station id */
	if (!test_staid(rtcm, staid)) return -1;

	strncpy(rtcm->sta.antType, des, n); rtcm->sta.antType[n] = '\0';
	rtcm->sta.antsetup = setup;
	rtcm->sta.antSerialNum[0] = '\0';
	return 5;
}
...
//		printf("rtcm3 1008 length error: len=%d\n", rtcm->len);
		return -1;
	}
	...
	strncpy(rtcm->sta.antType, des, n); rtcm->sta.antType[n] = '\0';
	rtcm->sta.antsetup = setup;
	strncpy(rtcm->sta.antSerialNum, sno, m); rtcm->sta.antSerialNum[m] = '\0';
	return 5;
}
...
static int decode_type1010(rtcm_t *rtcm)
{
	...
	for (j = 0; j<nsat&&rtcm->obs.n<MAXOBS&&i + 79 <= rtcm->len * 8; j++) {
		...
		cnr1 = getbitu(rtcm->buff, i, 8); i += 8;
		if (!(sat = satno2satIndex(sys, prn))) {
//			printf("rtcm3 1010 satellite number error: prn=%d\n", prn);
			continue;
		}
		tt = timediff(rtcm->obs.data[0].time, rtcm->time);
		if (rtcm->obsflag || fabs(tt)>1E-9) {
...
static int decode_type1012(rtcm_t *rtcm)
{
	...
	for (j = 0; j<nsat&&rtcm->obs.n<MAXOBS&&i + 130 <= rtcm->len * 8; j++) {
		...
		cnr2 = getbitu(rtcm->buff, i, 8); i += 8;
		if (!(sat = satno2satIndex(sys, prn))) {
//			printf("rtcm3 1012 satellite number error: sys=%d prn=%d\n", sys, prn);
			continue;
		}
		tt = timediff(rtcm->obs.data[0].time, rtcm->time);
		if (rtcm->obsflag || fabs(tt)>1E-9) {
...
static int decode_type1019(rtcm_t *rtcm)
{
	...
	if (i + 476 <= rtcm->len * 8) {
		prn = getbitu(rtcm->buff, i, 6);              i += 6;
		week = getbitu(rtcm->buff, i, 10);              i += 10;//??��??0~1023???????n*1024
		brdc.sva = getbitu(rtcm->buff, i, 4);              i += 4;
		brdc.code = getbitu(rtcm->buff, i, 2);              i += 2;
		brdc.idot = getbits(rtcm->buff, i, 14)*P2_43*PI; i += 14;
		brdc.iode = getbitu(rtcm->buff, i, 8);              i += 8;
		toc = getbitu(rtcm->buff, i, 16)*16.0;         i += 16;
		brdc.f2 = getbits(rtcm->buff, i, 8)*P2_55;        i += 8;
		brdc.f1 = getbits(rtcm->buff, i, 16)*P2_43;        i += 16;
		brdc.f0 = getbits(rtcm->buff, i, 22)*P2_31;        i += 22;
		brdc.iodc = getbitu(rtcm->buff, i, 10);              i += 10;
		brdc.crs = getbits(rtcm->buff, i, 16)*P2_5;         i += 16;
		brdc.deln = getbits(rtcm->buff, i, 16)*P2_43*PI; i += 16;
		brdc.M0 = getbits(rtcm->buff, i, 32)*P2_31*PI; i += 32;
		brdc.cuc = getbits(rtcm->buff, i, 16)*P2_29;        i += 16;
		brdc.e = getbitu(rtcm->buff, i, 32)*P2_33;        i += 32;
		brdc.cus = getbits(rtcm->buff, i, 16)*P2_29;        i += 16;
		sqrtA = getbitu(rtcm->buff, i, 32)*P2_19;        i += 32;
		brdc.toes = getbitu(rtcm->buff, i, 16)*16.0;         i += 16;
		brdc.cic = getbits(rtcm->buff, i, 16)*P2_29;        i += 16;
		brdc.OMG0 = getbits(rtcm->buff, i, 32)*P2_31*PI; i += 32;
		brdc.cis = getbits(rtcm->buff, i, 16)*P2_29;        i += 16;
		brdc.i0 = getbits(rtcm->buff, i, 32)*P2_31*PI; i += 32;
		brdc.crc = getbits(rtcm->buff, i, 16)*P2_5;         i += 16;
		brdc.omg = getbits(rtcm->buff, i, 32)*P2_31*PI; i += 32;
		brdc.OMGd = getbits(rtcm->buff, i, 24)*P2_43*PI; i += 24;
		brdc.tgd[0] = getbits(rtcm->buff, i, 8)*P2_31;        i += 8;
		brdc.svh = getbitu(rtcm->buff, i, 6);              i += 6;
		brdc.flag = getbitu(rtcm->buff, i, 1);              i += 1;
		//        brdc.fit = getbitu(rtcm->buff, i, 1) ? 0.0 : 4.0; /* 0:4hr,1:>4hr */
		brdc.fit = getbitu(rtcm->buff, i, 1); /* 0:4hr,1:>4hr edit by pwj 2018-4-19*/
	}
	else {
...
//		printf("rtcm3 1019 satellite number error: prn=%d\n", prn);
	...
	brdc.sat = sat;
	brdc.week = adjgpsweek(week);
	brdc.toe = gpst2time(brdc.week, brdc.toes);
	brdc.toc = gpst2time(brdc.week, toc);
	brdc.ttr = rtcm->time;
	brdc.A = sqrtA*sqrtA;

	if (brdc.fit <= 6) brdc.sva = (int)pow(2, (1 + brdc.fit) / 2);
	else if (brdc.fit > 6 && brdc.fit < 15) brdc.sva = (int)pow(2, brdc.fit - 2);
	else if (brdc.fit == 15) brdc.sva = 9999;
	if (brdc.A <= 0)
	{
		return -1;
	}

	if (brdc.iode == rtcm->nav.eph[sat - 1].iode)
		return 0; /* unchanged */

	rtcm->nav.eph[sat - 1] = brdc;
	rtcm->ephsat = sat;
	return 2;
}
...
//		printf("rtcm3 1020 length error: len=%d\n", rtcm->len);
		return -1;
	}
	if (!(sat = satno2satIndex(sys, prn))) {
//		printf("rtcm3 1020 satellite number error: prn=%d\n", prn);
		return -1;
	}
	trace(2, "decode_type1020: prn=%d tk=%02.0f:%02.0f:%02.0f\n", prn, tk_h, tk_m, tk_s);
...
//		printf("rtcm3 1033 length error: len=%d\n", rtcm->len);
		return -1;
	}
	if (rtcm->outtype) {
		msg = rtcm->msgtype + strlen(rtcm->msgtype);
	...
	strncpy(rtcm->sta.recVers, ver, n2); rtcm->sta.recVers[n2] = '\0';
	strncpy(rtcm->sta.recserialNum, rsn, n3); rtcm->sta.recserialNum[n3] = '\0';

	return 5;
}

/* decode type 63 and 1042: beidou ephemerides (rtcm draft) ---------------------------*/
...
//		printf("rtcm3 1045 length error: len=%d\n", rtcm->len);
		return -1;
	}
	...
	if (!(sat = satno2satIndex(sys, prn))) {
//		printf("rtcm3 1045 satellite number error: prn=%d\n", prn);
		return -1;
	}
	eph.sat = sat;
...
//		printf("rtcm3 1047 length error: len=%d\n", rtcm->len);
		return -1;
	}
	...
	if (!(sat = satno2satIndex(sys, prn))) {
//		printf("rtcm3 1047 satellite number error: prn=%d\n", prn);
		return -1;
	}
	eph.sat = sat;
	eph.week = adjbdtweek(week);
...
static int decode_ssr1_head(rtcm_t *rtcm, int sys, int *sync, int *iod, double *udint, int *refd, int *hsize)
{
	...
	if (i + (sys == SYS_GLO ? 53 : 56) > rtcm->len * 8)
	{
		printf("ssr head length error!\n");
		return -1;
	}
	if (sys == SYS_GLO)
	{
	...
	else
	{
		...
		i += 20;
		adjweek_gpst(rtcm, tow);
	}
	udi = getbitu(rtcm->buff, i, 4); i += 4;
	*sync = getbitu(rtcm->buff, i, 1); i += 1;
	*refd = getbitu(rtcm->buff, i, 1); i += 1;
...
//??1057???��Satellite Reference Datum
...
static int decode_ssr7_head(rtcm_t *rtcm, int *sync, int *iod, double *udint, int *vtecqi, int *hsize)
{
	double tow;
	int i = 24 + 12, udi, noil = 0, provid = 0, solid = 0;
	...
	*vtecqi = getbitu(rtcm->buff, i, 9); i += 9;
	noil = getbitu(rtcm->buff, i, 2); i += 2;
	*udint = ssrudint[udi];
	*hsize = i;

	rtcm->ion.iod = *iod;
	rtcm->ion.udint = *udint;
	rtcm->ion.vtecqi = *vtecqi;
	rtcm->ion.provid = provid;
	rtcm->ion.solid = solid;
	rtcm->ion.nlayer = noil;
	return noil;
}

//decode ssr??RTCM1057 GPS?????????IODE???��???.
...
static int numofcosc(int d, int o)
{
	int N = d + 1, M = o + 1;
	return (N + 1)*(N + 2) / 2 - (N - M)*(N - M + 1) / 2;
}

static int numofsinc(int d, int o)
{
	int N = d + 1, M = o + 1;
	return (N + 1)*(N + 2) / 2 - (N - M)*(N - M + 1) / 2 - (N + 1);
}
/*VTEC from CNES 1264*/
static int decode_ssr7(rtcm_t *rtcm)
{
	int i, j, k, type, noil, sync, iod, vtecqi;
	double udint;
	int height, degree, order, cosc, sinc, ncosc, nsinc, np = 0;
	type = getbitu(rtcm->buff, 24, 12);
	if ((noil = decode_ssr7_head(rtcm, &sync, &iod, &udint, &vtecqi, &i)) < 0
		&& vtecqi>511) {/*0~3 as 1~4            5110=25.55TECU/0.05TECU*/
//		printf("rtcm3 %d length error: len=%d\n", type, rtcm->len);
		return -1;
	}


	for (j = 0; j <= noil&&j < MAXTECLAYER&&i + 16 <= rtcm->len * 8; j++)
	{
		height = getbitu(rtcm->buff, i, 8); i += 8; np++;
		degree = getbitu(rtcm->buff, i, 4); i += 4; np++;
		order = getbitu(rtcm->buff, i, 4); i += 4; np++;
		ncosc = numofcosc(degree, order);
		nsinc = numofsinc(degree, order);

		rtcm->ion.vtec[j][0] = height*1E4;
		rtcm->ion.vtec[j][1] = degree + 1;
		rtcm->ion.vtec[j][2] = order + 1;

		for (k = 0; k < ncosc; k++, np++)
		{
			cosc = getbits(rtcm->buff, i, 16); i += 16;
			if (fabs(cosc*0.005) > 163.85)
				rtcm->ion.vtec[j][np] = 999.0;//set error index
			else
				rtcm->ion.vtec[j][np] = cosc*0.005;
		}
		for (k = 0; k < nsinc; k++, np++)
		{
			sinc = getbits(rtcm->buff, i, 16); i += 16;
			if (fabs(sinc*0.005) > 163.85)
				rtcm->ion.vtec[j][np] = 999.0;//set error index
			else
				rtcm->ion.vtec[j][np] = sinc*0.005;
		}
	}
	rtcm->ion.flag = 1;
	rtcm->ion.time = rtcm->time;
...
//			printf("rtcm3 %d: unknown signal id=%2d\n", type, h->sigs[i]);
	...
	for (i = j = 0; i<h->nsat; i++) {
		...
		if (sys == SYS_SBS) prn += MINPRNSBS - 1;
...
//			printf("rtcm3 %d satellite error: prn=%d\n", type, prn);
		...
		for (k = 0; k<h->nsig; k++) {
			...
			if (sat&&index >= 0 && ind[k] >= 0) {
				...
				wl = GetSatWaveLength(sat, freq[k] - 1, &rtcm->nav);
				...
				rtcm->obs.data[index].sys = sys;
				strcpy(rtcm->obs.data[index].satID, satIndex2satID(satID, sat));
			}
			j++;
		}
	}
...
//		printf("rtcm3 %d length error: len=%d nsat=%d nsig=%d\n", type,
...
//		printf("rtcm3 %d length error: nsat=%d ncell=%d len=%d\n", type, h.nsat,
//			ncell, rtcm->len);
		return -1;
	}
...
extern int rtcmDecode(rtcm_t *rtcm, unsigned char data)
{
	...
	//{
	//	type = getbitu(rtcm->buff, 24, 12);
	//	if ((type > 1400 || type < 1000) && (type<4000 || type>4400))
	//	{
	//		rtcm->nbyte = 0;
	//		return 0;
	//	}
...
extern int Decode(rtcm_t *rtcm)
{
	...
	//	rtcm->time.time = (time_t)time2gpst(timeadd(timeget(),leapsecond(timeget()))).sec;
	switch (type) {
	...
	case 1009: ret = decode_type1009(rtcm); break;
	case 1010: ret = decode_type1010(rtcm); break;
	case 1011: ret = decode_type1011(rtcm); break;
	...
	case 1047: ret = decode_type1047(rtcm); break;
...
//	case 1125: ret = decode_msm5(rtcm, SYS_CMP); break;
	...
	case 1264: ret = decode_ssr7(rtcm); break;
